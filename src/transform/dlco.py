# src/transform/dlco.py
"""Transforma la extracción DLCO en una tabla analítica por visita."""
...
from src.config.dlco import (
    DLCO_NUMERIC_COLUMNS,
    DLCO_SQL_COLUMNS,
)


# Parámetros funcionales que se transformarán a formato ancho.
DLCO_PARAMETERS = list(DLCO_NUMERIC_COLUMNS)
...
def build_dlco_measurements(raw_dataframe: pd.DataFrame) -> pd.DataFrame:
    """Convierte los componentes DLCO a formato ancho por visita."""
    ...
    numeric_columns = [
        f"{parameter.lower()}_{component}"
        for parameter in DLCO_NUMERIC_COLUMNS
...
def add_derived_metrics(
    ...
    for parameter in DLCO_PARAMETERS:
        ...
        for moment in ["pre", "post"]:
            ...
            zscore_column = f"{prefix}_zscore_{moment}"
            zscore = pd.Series(
                np.nan,
                index=transformed.index,
                dtype="float64",
            ...
            if (
                predicted_column in transformed.columns
                and sd_column in transformed.columns
            ):
                predicted = pd.to_numeric(
                    transformed[predicted_column],
                    errors="coerce",
                ...
                zscore.loc[valid_sd_formula] = (
                    observed.loc[valid_sd_formula]
                    - predicted.loc[valid_sd_formula]
                ) / sd.loc[valid_sd_formula]

            derived_columns[zscore_column] = zscore
...
def transform_dlco(
    ...
    """Construye la tabla analítica final de DLCO."""
    ...
    measurement_dataframe = build_dlco_measurements(
        raw_dataframe
    )
